...
`aiqa` — an AI QA Copilot for CI/CD. A drop-in pipeline that reviews code on every push with an LLM — **with cross-file context** so it catches integration bugs, not just single-file ones — surfaces real defects and reliability risks, **generates the missing tests as real `.feature` + pytest files**, and **self-heals broken UI selectors** into resilient Playwright locators. Publishes a shareable HTML report with a weighted risk score and a pass/fail gate. A real run on a 3-file demo API surfaced **21 findings (5 critical)** — the [live report](https://priya123z.github.io/AI-pipeline-report/report/) is that run. Ships as a CLI, a GitHub Action, and a Python library; every LLM response is Pydantic-validated, and the suite runs green with **no API key** (LLM mocked). Free on any OpenRouter model.
